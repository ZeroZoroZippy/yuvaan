...
import { db, hasFirebaseConfig, firebaseReady } from '../config/firebase';
...
class AnalyticsService {
    ...
    async initializeSession() {
        if (!hasFirebaseConfig || !db) {
            if (process.env.NODE_ENV === 'development') {
                console.log('📊 Analytics: Session initialized (offline mode)');
            }
            return;
        }
    ...
    async uploadBatchInteractions(isSync = false) {
        ...
        if (!hasFirebaseConfig || !db) {
            if (process.env.NODE_ENV === 'development') {
                console.log('📊 Analytics: Batch upload skipped (offline mode)', {
        ...
        try {
            ...
            console.error('Error uploading batch interactions:', error);
    ...
    async updateClickCounter(counterType, itemName) {
        if (!hasFirebaseConfig || !db) {
            if (process.env.NODE_ENV === 'development') {
                console.log('📊 Analytics: Counter update skipped (offline mode)', { counterType, itemName });
            }
            return;
        }
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('📊 Analytics: Counter updated', { counterType, itemName });
            }
        } catch (error) {
